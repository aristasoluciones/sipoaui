...
import ImportCatalogDialog from '@/src/components/catalogos/ImportCatalogDialog';
...
const PartidasPresupuestariasPage = () => {
    ...
    const filteredCapitulos = capitulos.filter((capitulo) => {
        ...
        return (capitulo.codigo?.toLowerCase() || '').includes(searchLower) || (capitulo.nombre?.toLowerCase() || '').includes(searchLower);
    });

    const filteredPartidas = partidas.filter((partida) => {
        if (!filterPartidas) return true;
        const searchLower = filterPartidas.toLowerCase();
        return (partida.codigo?.toLowerCase() || '').includes(searchLower) || (partida.nombre?.toLowerCase() || '').includes(searchLower);
    });
    ...
    // Exportar a CSV todas las partidas (todos los capítulos)
    const handleExportCSV = () => {
        if (capitulos.length === 0) {
            error('No hay datos para exportar');
            return;
        }
        const rows: string[][] = [['Código Capítulo', 'Nombre Capítulo', 'Código Partida', 'Nombre Partida']];
        // Si hay capítulo seleccionado, solo exportar esas partidas; si no, todas las cargadas
        const exportPartidas = partidas.length > 0 ? partidas : [];
        exportPartidas.forEach((p) => {
            rows.push([selectedCapitulo?.codigo ?? '', selectedCapitulo?.nombre ?? '', p.codigo ?? '', p.nombre ?? '']);
        });
        if (rows.length === 1) {
            error('Selecciona un capítulo para exportar sus partidas');
            return;
        }
        ...
        a.href = url;
        a.download = `partidas_${selectedCapitulo?.codigo ?? 'todas'}_${new Date().toISOString().slice(0, 10)}.csv`;
    ...
    const capituloActionsTemplate = (rowData: Capitulo) => {
        ...
        return (
            <div className="flex gap-1">
                {canUpdate && <Button icon="pi pi-pencil" text size="small" severity="success" onClick={() => editCapitulo(rowData)} tooltip="Editar" />}
                {canUpdate && (
                    <Button
                ...
                {canDelete && <Button icon="pi pi-trash" text size="small" severity="danger" onClick={() => deleteCapitulo(rowData)} tooltip="Eliminar" />}
            </div>
        );
    };
    ...
    const partidaActionsTemplate = (rowData: Partida) => {
        ...
        return (
            <div className="flex gap-1">
                {canUpdate && <Button icon="pi pi-pencil" text size="small" severity="success" onClick={() => editPartida(rowData)} tooltip="Editar" />}
                {canUpdate && (
                    <Button
                        icon={rowData.estado === 'Activo' ? 'pi pi-eye-slash' : 'pi pi-eye'}
                        text
                ...
                {canDelete && <Button icon="pi pi-trash" text size="small" severity="danger" onClick={() => deletePartida(rowData)} tooltip="Eliminar" />}
            </div>
        );
    };
        ...
        <>
            ...
            <div className="grid">
                    ...
                    <div className="card mb-3">
                        <div className="flex justify-content-between align-items-start">
                            <div className="flex-grow-1">
                                ...
                                <p className="text-600 mt-2 mb-0">Gestión de capítulos y partidas presupuestarias del sistema</p>
                            </div>
                            <Button label="Regresar a Catálogos" icon="pi pi-arrow-left" outlined onClick={() => router.push('/catalogos')} />
                            <Button label="Importar" icon="pi pi-upload" severity="secondary" tooltip="Importar capítulos y partidas desde Excel" tooltipOptions={{ position: 'bottom' }} onClick={() => setShowImportDialog(true)} />
                            <Button label="Exportar" icon="pi pi-download" severity="help" tooltip="Exportar partidas del capítulo seleccionado a CSV" tooltipOptions={{ position: 'bottom' }} onClick={handleExportCSV} />
                        </div>
                    </div>
                </div>
                                ...
                                <div className="p-3">
                                    <div className="flex justify-content-between align-items-center mb-3">
                                        ...
                                        <div className="flex gap-2">
                                            {canCreate && <Button label="Nuevo Capítulo" icon="pi pi-plus" size="small" onClick={openNewCapitulo} />}
                                            <Button icon="pi pi-refresh" severity="secondary" size="small" onClick={loadCapitulos} tooltip="Actualizar" />
                                        </div>
                                    </div>
                                    <div className="flex justify-content-end">
                                        <span className="p-input-icon-left">
                                            <i className="pi pi-search" />
                                            <InputText placeholder="Buscar capítulo..." value={filterCapitulos} onChange={(e) => setFilterCapitulos(e.target.value)} />
                                        </span>
                                    </div>
                                </div>
                            }
                    ...
                    {selectedCapitulo ? (
                                ...
                                header={
                                    <div className="p-3 bg-primary-50 border-bottom-1 border-primary-100">
                                        <div className="flex justify-content-between align-items-center">
                                            ...
                                            <div className="flex gap-2">
                                                {canCreate && <Button label="Nueva Partida" icon="pi pi-plus" size="small" onClick={openNewPartida} />}
                                                <Button icon="pi pi-refresh" severity="secondary" size="small" onClick={() => selectedCapitulo && loadPartidas(selectedCapitulo.id)} tooltip="Actualizar" />
                                            </div>
                                        </div>
                                        <div className="flex justify-content-end mt-3">
                                            <span className="p-input-icon-left">
                                                <i className="pi pi-search" />
                                                <InputText placeholder="Buscar partida..." value={filterPartidas} onChange={(e) => setFilterPartidas(e.target.value)} />
                                            </span>
                                        </div>
                                    </div>
                                }
                        ...
                        <div className="card">
                            <div className="flex flex-column align-items-center justify-content-center py-8">
                                ...
                                <p className="text-600 text-center">Seleccione un capítulo de la lista izquierda para ver sus partidas</p>
                            </div>
                        </div>
                    )}
                </div>
                ...
                <Dialog
                    ...
                    header={
                        <div>
                            <span className="text-xl font-semibold">{editingCapitulo?.id ? 'Editar Capítulo' : 'Nuevo Capítulo'}</span>
                            <p className="text-600 text-sm mt-2 mb-0">{editingCapitulo?.id ? 'Modifica la información del capítulo' : 'Completa los campos para crear un nuevo capítulo'}</p>
                        </div>
                    }
                    modal
                    className="p-fluid"
                    footer={
                        <div className="flex justify-content-end gap-2">
                            <Button label="Cancelar" icon="pi pi-times" severity="secondary" outlined onClick={() => setShowCapituloDialog(false)} />
                            <Button label="Guardar" icon="pi pi-check" onClick={saveCapitulo} loading={saving} />
                        </div>
                    }
                ...
                >
                    <div className="grid p-3">
                        <div className="col-12">
                            ...
                            <InputText id="codigo" value={editingCapitulo?.codigo || ''} onChange={(e) => setEditingCapitulo((prev) => ({ ...prev!, codigo: e.target.value }))} placeholder="Ej: 1000" />
                        </div>
                        ...
                        <div className="col-12">
                            ...
                            <InputText id="nombre" value={editingCapitulo?.nombre || ''} onChange={(e) => setEditingCapitulo((prev) => ({ ...prev!, nombre: e.target.value }))} placeholder="Ej: Servicios Personales" />
                        </div>
                ...
                <Dialog
                    ...
                    header={
                        <div>
                            <span className="text-xl font-semibold">{editingPartida?.id ? 'Editar Partida' : 'Nueva Partida'}</span>
                            <p className="text-600 text-sm mt-2 mb-0">{editingPartida?.id ? 'Modifica la información de la partida' : 'Completa los campos para crear una nueva partida'}</p>
                        </div>
                    }
                    modal
                    className="p-fluid"
                    footer={
                        <div className="flex justify-content-end gap-2">
                            <Button label="Cancelar" icon="pi pi-times" severity="secondary" outlined onClick={() => setShowPartidaDialog(false)} />
                            <Button label="Guardar" icon="pi pi-check" onClick={savePartida} loading={saving} />
                        </div>
                    }
                ...
                >
                    <div className="grid p-3">
                        <div className="col-12">
                            ...
                            <InputText id="codigo-partida" value={editingPartida?.codigo || ''} onChange={(e) => setEditingPartida((prev) => ({ ...prev!, codigo: e.target.value }))} placeholder="Ej: 1101" />
                        </div>
                        ...
                        <div className="col-12">
                            ...
                            <InputText id="nombre-partida" value={editingPartida?.nombre || ''} onChange={(e) => setEditingPartida((prev) => ({ ...prev!, nombre: e.target.value }))} placeholder="Ej: Sueldos Base" />
                        </div>
            ...
            <ImportCatalogDialog
                ...
                onRefresh={loadCapitulos}
                catalogKey="partidas_capitulos"
                catalogTitle="Importar Tabulador de Partidas Presupuestarias"
                description="Selecciona el archivo Excel del Tabulador de Partidas. El sistema detectará automáticamente los capítulos (códigos terminados en 000) y sus partidas asociadas."
            />
        </>
    );
};
